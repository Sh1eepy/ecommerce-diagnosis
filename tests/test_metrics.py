...
def test_cvr_and_rates():
    row = {"uv": 1000, "view_count": 3000, "addtocart_count": 90, "transaction_count": 80}
    out = compute_metrics(row, ["cvr", "addcart_rate", "cart_to_pay", "gmv", "click_rate", "per_user_views"])
    assert out["cvr"] == 8.0
...
def test_division_by_zero_returns_zero():
    row = {"uv": 0, "view_count": 0, "addtocart_count": 0, "transaction_count": 0}
    out = compute_metrics(row, ["cvr", "cart_to_pay", "click_rate"])
    assert out["cvr"] == 0.0
