...
def query_metric(item_id, start_date, end_date, metrics=None):
...
def MetricTool() -> StructuredTool:
    """返回原生 LangChain 工具；业务入口统一由注册表校验与审计。"""
    return StructuredTool(name=name, description=description, args_schema=parameters, func=query_metric)
